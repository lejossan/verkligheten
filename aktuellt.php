<?php $title = "Saker som händer"; include('header.php'); ?>

  <div class="imagewrapper--small brollop">
    <div class="header-image image"></div>
    <h1>SAKER SOM HÄNDER</h1>
  </div><div class="menu">
      <ul class="container">
        <li class="h3 title">MENY</li>
        <li class="h3"><a href="index.php">HEM</a></li>
        <li class="h3"><a href="far.php">FÅR</a></li>
        <li class="h3"><a href="honor.php">HÖNS</a></li>
        <li class="h3"><a href="halmhus.php">HALMHUS</a></li>
        <li class="h3 active">AKTUELLT</li>
        <li class="h3"><a href="folkochfa.php">FOLK & FÄ</a></li>
        <li class="h3"><a href="#footer">KONTAKT</a></li>
      </ul>
  </div>
  

  <div class="section container">
    <div class="col--6">
      <h2>Välkommen till adventsmys i Verkligheten</h2>

      <p>Den andra advent bjuder vi in till adventsmys, en julmarknad i miniformat här på Verklighetens Gård. Logen är pyntad till julstämning och vi bjuder på glögg och pepparkaka, det finns även försäljning i liten skala och såklart fika. I vår butik finns bland annat miljövänliga rengöringsmedel, tvålar, disk- och skurborstar men även saker vi tovat och grönsaker vi odlat. Det kommer även finnas inbjudna hantverkare som säljer sina alster och chans att hitta miljövänliga julklappar eller gå-bort presenter till julens fester.</p>

      <p>Här bygger vi även ett halmbalshus, Lilla Verkligheten. Vi vet att intresset är stort och öppnar därför upp huset för visning en sista gång innan det är dags för inflytt. Kolla gärna in instagramkontot <a href="https://www.instagram.com/byggiverkligheten/" target="_blank">@byggiverkligheten</a> för att få mer information om huset. </p>

      <p>Gratis parkering och inträde!</p>

      <p class="strong">Nästa julmarknad äger rum 9 december 2018!</p>
    </div> 
    <div class="col--6">
      <br/>
      <br/>
      <br/>
      <br/>
      <a href="../img/advent/hylla.png" data-fancybox="advent" data-caption="Folly hjälper till att dra hem granar">
        <img class="" src="../img/advent/hylla-thumb.png" alt="folly" />
      </a>
    </div>
  </div>
  <div class="section container">
    <div class="col--2">
      <a href="../img/advent/folly.png" data-fancybox="advent" data-caption="Folly hjälper till att dra hem granar">
        <img class="" src="../img/advent/folly-thumb.png" alt="folly" />
      </a>
    </div><!--
    --><div class="col--2">
      <a href="../img/advent/bord.png" data-fancybox="advent" data-caption="Bord med godsaker">
        <img class="" src="../img/advent/bord-thumb.png" alt="Bord med godsaker" />
      </a>
    </div><!--
    --><div class="col--2">
        <a href="../img/advent/kit.png" data-fancybox="advent" data-caption="Adventskit med ljus och mossa.">
          <img class="" src="../img/advent/kit-thumb.png" alt="kit" />
        </a>
    </div><!--
    --><div class="col--2">
      <a href="../img/advent/godis.png" data-fancybox="advent" data-caption="Godis">
        <img class="" src="../img/advent/godis-thumb.png" alt="Godis" />
      </a>
    </div><!--    
    --><div class="col--2">
      <a href="../img/advent/fonster.png" data-fancybox="advent" data-caption="Hela logen är pyntad!">
        <img class="" src="../img/advent/fonster-thumb.png" alt="Fönster" />
      </a>
    </div><!--    
    --><div class="col--2">
        <a href="../img/advent/tomtar.png" data-fancybox="advent" data-caption="Tovade tomtar till försäljning">
          <img class="" src="../img/advent/tomtar-thumb.png" alt="Tomtar" />
        </a>
    </div>
  </div>
  <div class="section container">
    <div class="col--6">

      <h2>VÅRMARKNAD</h2>
      <p>Kom och hälsa på våra djur, ta en fika i trädgården eller kolla in vår lilla loppis. I stallet hittar du en liten butik med bland annat ekologiska rengöringsprodukter, tvålar, tovat och säkert några grönsaksplantor.  </p>

      <p>Missa heller inte att kika på vad våra inbjudna försäljare har gjort. Hantverk i garn, tyg och trä, egensydda barnkläder, luffarslöjd och säkert fler saker kommer också att finnas på borden.  </p>

      <p>Allt i liten skala och med omtanke om människor och miljö.</p>

      <!--<p class="strong">Nästa vårmarknad äger rum 2019!</p>-->
    </div> 
    <div class="col--6">
      <br/>
      <br/>
      <br/>
      
      <a href="../img/varmarknad.png" data-fancybox="varmarknad" data-caption="Vårmarknad">
        <img class="" src="../img/varmarknad.png" alt="varmarknad" />
      </a>
    </div>
  </div>




<?php include('footer.php'); ?>